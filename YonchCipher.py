...
def coord_finder(words, letter):
    valid_indices = [i for i, word in enumerate(words) if letter in word]

    if not valid_indices:
        return None

    num = random.choice(valid_indices)
    return [num, words[num].index(letter)]
...
def YonchEncrypt(text, word_list):
    encrypt = ""
    for letter in text:
...
def YonchDecrypt(text, word_list):
    decrypt = ""
...
text = YonchEncrypt("hello beautiful world!", words)
print(text)
decrypted = YonchDecrypt(text, words)
print(decrypted)
